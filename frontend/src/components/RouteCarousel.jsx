...
function UpcomingTripCard({ trip, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-xl p-5 border border-outline-variant hover:border-primary transition-colors cursor-pointer flex flex-col gap-3"
    >
      <div className="flex items-center justify-between">
        <p className="font-bold text-on-surface text-base leading-tight">
          {trip.fromCity} → {trip.toCity}
        </p>
        <span className="text-xs font-semibold text-secondary bg-surface-container-low px-2 py-0.5 rounded-md">
          Xe {trip.totalSeats} chỗ
        </span>
      </div>

      <p className="text-xs text-secondary truncate">{trip.companyName}</p>

      <div className="flex items-end justify-between gap-2">
        <div>
          <p className="text-2xl font-bold text-on-surface">{formatTime(trip.departureTime)}</p>
        ...
        <div className="text-right">
          ...
          <p className={`text-xs font-semibold ${low ? "text-red-500" : "text-green-600"}`}>
  ...
  return (
    <div className="relative sm:px-12">
      <button
        ...
        className="hidden sm:flex absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-sm border border-outline-variant items-center justify-center hover:bg-surface-container-low transition-colors"
      >
      ...
      <button
        ...
        className="hidden sm:flex absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 rounded-full bg-white shadow-sm border border-outline-variant items-center justify-center hover:bg-surface-container-low transition-colors"
      >
